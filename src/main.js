...
export function reduceState(state, action, random = Math.random) {
  ...
  if (action.type === 'set-difficulty') return nextQuestion(state, random, { difficulty: action.difficulty });
...
function lessonControls(state) {
  const directions = state.lessonId === 'number-line' ? `<button type="button" data-direction="number-to-point" aria-pressed="${state.options.direction === 'number-to-point'}">Number → Point</button><button type="button" data-direction="point-to-number" aria-pressed="${state.options.direction === 'point-to-number'}">Point → Number</button>` : '';
  return `<div class="config-controls" aria-label="Question settings">${directions}<button type="button" data-difficulty="easy" aria-pressed="${state.options.difficulty === 'easy'}">Easy</button><button type="button" data-difficulty="challenge" aria-pressed="${state.options.difficulty === 'challenge'}">Challenge</button></div>`;
}
    ...
    app.addEventListener('click', (event) => {
      ...
      const action = button.dataset.lesson ? { type: 'select-lesson', lessonId: button.dataset.lesson }
        : button.dataset.direction ? { type: 'set-number-line-direction', direction: button.dataset.direction }
          : button.dataset.difficulty ? { type: 'set-difficulty', difficulty: button.dataset.difficulty }
